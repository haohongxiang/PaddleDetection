...
__all__ = ['DETRLoss', '_hungarian_matcher']



@paddle.no_grad()
def _hungarian_matcher(boxes, scores, gt_bbox, gt_class):
    r"""
    Args:
...
#         bbox_cxcywh_to_xyxy(tgt_bbox.unsqueeze(0))).squeeze(-1) - 1
    cost_giou = GIoULoss()(bbox_cxcywh_to_xyxy(out_bbox.unsqueeze(1)), bbox_cxcywh_to_xyxy(tgt_bbox.unsqueeze(0))).squeeze(-1) - 1
...
class DETRLoss(nn.Layer):
    ...
    def _get_loss_class(self, scores, gt_class, match_indices, bg_index):
        ...
        print(scores.shape, target_label.shape, scores.transpose([0, 2, 1]).shape)
        print(scores.mean(), scores.sum())
        # print(self.loss_coeff['class'])
        print(target_label)
    ...
    def _get_loss_bbox(self, boxes, gt_bbox, match_indices):
        ...
        # print( F.l1_loss(src_bbox, target_bbox, reduction='none').sum() )
        # print( F.l1_loss(src_bbox, target_bbox, reduction='sum') )

        # print('loss_coeff', self.loss_coeff['bbox'])
        # print('num_gts', num_gts)
        
        loss = dict()
        loss['loss_bbox'] = self.loss_coeff['bbox'] * F.l1_loss(
            src_bbox, target_bbox, reduction='sum') / num_gts
#         loss['loss_bbox'] = F.l1_loss(src_bbox, target_bbox, reduction='sum') / num_gts

        # print("loss['loss_bbox']", loss['loss_bbox'])
        ...
        # print("loss['loss_giou']", loss['loss_giou'])
    ...
    def forward(self,
        ...
        total_loss.update(
            self._get_loss_class(scores[-1], gt_class, match_indices, self.num_classes))
