...
class Conv(nn.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        param_attr = paddle.ParamAttr(regularizer=paddle.regularizer.L2Decay(0.0005),
                               trainable=True)
        self.conv = nn.Conv2D(c1, c2, k, s, autopad(k, p),
                              groups=g, weight_attr=param_attr)
...
class Expand(nn.Layer):
    ...
    def forward(self, x):
        ...
        return x.view(N, C // s ** 2, H * s, W * s)  # x(1,16,160,160)ed_parameters():
