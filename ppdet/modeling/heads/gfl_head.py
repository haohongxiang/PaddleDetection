...
#         self.loss_bbox = loss_bbox
#         self.reg_max = reg_max
#         self.feat_in_chan = [feat_in_chan, ] * len(fpn_stride) if isinstance(feat_in_chan, int) else feat_in_chan
#         self.nms = nms
#         self.nms_pre = nms_pre
#         self.cell_offset = cell_offset
#         self.use_sigmoid = self.loss_qfl.use_sigmoid
...
#         self.gfl_head_cls = []
#         for i in range(len(self.feat_in_chan)):
#             m = self.add_sublayer(
#                 conv_cls_name + f'_{i}',
#                 nn.Conv2D(
...
#         cls_logits_list = []
#         bboxes_reg_list = []
#         for i, (scale_reg, fpn_feat) in enumerate(zip(self.scales_regs, fpn_feats)):
#             conv_cls_feat, conv_reg_feat = self.conv_feat(fpn_feat)

#             cls_logits = self.gfl_head_cls[i](conv_cls_feat)
#             bbox_reg = scale_reg(self.gfl_head_reg[i](conv_reg_feat))
