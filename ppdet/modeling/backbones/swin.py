...
__all__ = ['SwinTransformer', 'WindowAttention']


trunc_normal_ = TruncatedNormal(std=.02)  # 初始化
zeros_ = Constant(value=0.)
ones_ = Constant(value=1.)
normal_ = Normal(mean=0.,std=0.001)
...
#         print('-----------')
        ...
        print('qk_scale', qk_scale, 'head_dim', head_dim, )

        # self.scale = head_dim ** -0.5 or qk_scale 
        # print(f'{head_dim} ** -0.5 or {qk_scale} == ', self.scale)
        ...
        assert qk_scale is None, f'{qk_scale}'
        assert head_dim == 32, f'{head_dim}'
        
        x = None or 32 ** -0.5 
        
        print(f'{qk_scale} or {head_dim} ** -0.5 ==', self.scale, x)
        ...
        self.relative_position_bias_table = self.create_parameter(
            ...
            default_initializer=nn.initializer.Constant(value=0))  # 2*Wh-1 * 2*Ww-1, nH
        ...
        ## -----------------end---------  ##
        self.qkv = nn.Linear(dim, dim * 3, bias_attr=qkv_bias)  # *3
    ...
    def forward(self, x, mask=None):
        ...
        """
        B, N, C = x.shape
        qkv = self.qkv(x).reshape((B, N, 3, self.num_heads, C // self.num_heads)).transpose((2, 0, 3, 1, 4))  # 3次线性变换
        ...
        relative_position_bias = paddle.index_select(self.relative_position_bias_table,
                                                     self.relative_position_index.reshape((-1,)), axis=0).reshape(
...
def window_reverse(windows, window_size, H, W):
    ...
    #print(B,H,W,window_size, H // window_size, W // window_size)
...
class SwinTransformerBlock(nn.Layer):
    ...
    def forward(self, x, mask_matrix):
        ...
        # pad feature maps to multiples of window size #[B,H+pad_t+pad_b,W+pad_l+pad_r,C]
        ...
        #print(x_windows.shape, mask_matrix.shape)
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=attn_mask)  # nW*B, window_size*window_size, C
        #print(attn_windows.shape, x_windows.shape)
        # merge windows
        attn_windows = attn_windows.reshape((-1, self.window_size, self.window_size, C))
        #print(attn_windows.shape,self.window_size, H, W, Hp, Wp)
...
class SwinTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        if self.ape:
            ...
            self.absolute_pos_embed = self.create_parameter(shape=(1, embed_dim,  patches_resolution[0], patches_resolution[1]),
        ...
        # stochastic depth
        # dpr = [x for x in paddle.linspace(0, 0, sum(depths))]  # stochastic depth decay rule
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.return_idx:
                ...
                out = x_out.reshape((-1, H, W, self.feat_channels[i])).transpose((0, 3, 1, 2))
                outs.append(out)
    ...
    def out_shape(self):
        return [
            ShapeSpec(
                channels=self.feat_channels[i], stride=self._out_strides[i])
            for i in self.return_idx
        ]
# 
# def swin_base_patch4_window7_512(**kwargs):
...
#     x = (1, 3, 512, 512)  # nchw
#     x_data = paddle.rand(x, dtype='float32')
#     inputs = paddle.to_tensor(x_data)
#     model = swin_base_patch4_window7_512()
