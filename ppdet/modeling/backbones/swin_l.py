...
class SwinTransformerBlock(nn.Layer):
    ...
    def forward(self, x, mask_matrix, shift_size=None):
        """ Forward function.
        Args:
        ...
        if shift_size > 0:
            shifted_x = paddle.roll(
                x, shifts=(-shift_size, -shift_size), axis=(1, 2))
        ...
        if shift_size > 0:
            x = paddle.roll(
                shifted_x, shifts=(shift_size, shift_size), axis=(1, 2))
        else:
            x = shifted_x
...
class BasicLayer(nn.Layer):
    ...
    def forward(self, x, H, W):
        ...
        for blk in self.blocks:
            blk.H, blk.W = H, W
            x = blk(x, attn_mask, shift_size)  # TODO
