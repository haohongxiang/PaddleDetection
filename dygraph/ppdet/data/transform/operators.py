...
class Mosaic(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        assert im.shape[:2] == (self.img_size, self.img_size), ''

        return result

    def load_mosaic(self, sample, img_size=640):
        '''load mosaic
        '''
        if not isinstance(sample, Sequence):
            return sample
        ...
        img4, labels4 = self.random_perspective(
            img=img4,
            targets=labels4,
            degrees=self.degree,
        ...
        if self.hsv_h > 0 or self.hsv_s > 0 or self.hsv_v > 0:
            self.augment_hsv(img4, self.hsv_h, self.hsv_s, self.hsv_v)
    ...
    def augment_hsv(img, hgain=0.5, sgain=0.5, vgain=0.5):
        r = np.random.uniform(-1, 1,
                              3) * [hgain, sgain, vgain] + 1  # random gains
