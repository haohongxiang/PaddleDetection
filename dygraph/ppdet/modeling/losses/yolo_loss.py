...
class YOLOv3Loss(nn.Layer):
    ...
    def obj_loss(self, pbox, gbox, pobj, tobj, anchor, downsample):
        ...
        return (loss_obj_pos + loss_obj_neg).sum() / (iou_mask.sum() + obj_mask.sum() + 1e-5) , iou_mask
    ...
    def yolov3_loss(self, p, t, gt_box, anchor, downsample, scale=1.,
        ...
        num_obj = (tobj > 0).astype(p.dtype).sum() + 1e-5

        loss_xy = loss_xy.sum() / num_obj  #sum([1, 2, 3, 4]).mean()

        loss_w = paddle.abs(w - tw)
        loss_h = paddle.abs(h - th)
        loss_wh = tscale_obj * (loss_w + loss_h)
        loss_wh = loss_wh.sum() / num_obj #sum([1, 2, 3, 4]).mean()

        loss['loss_xy'] = loss_xy * 0.05 / 2 * b
        loss['loss_wh'] = loss_wh * 0.05 / 2 * b

        box = [x, y, w, h]
        loss_obj, iou_mask = self.obj_loss(box, gt_box, obj, tobj, anchor, downsample)
        loss['loss_obj'] = loss_obj * b

        loss_cls = self.cls_loss(pcls, tcls) * tobj 
        loss['loss_cls'] = loss_cls.sum() / 80. / num_obj * 0.5 * b #  num_obj

        # loss_cls = loss_cls.sum([1, 2, 3, 4]).mean()
        # loss['loss_cls'] = loss_cls.mean() * b * 10 #  num_obj

        # loss_cls = self.cls_loss(pcls, tcls) * (1 - iou_mask).reshape(shape=pcls.shape[:-1]+[1,])
        # loss['loss_cls'] = loss_cls.sum() / ((1 - iou_mask).sum() + 1e-5) / 80 * b * 0.5 #  num_obj
        ...
        if self.iou_loss is not None:
            ...
            # loss_iou = loss_iou.mean() * b  #sum([1, 2, 3, 4]).mean()
            loss_iou = loss_iou.sum() / num_obj * 0.05 * b #sum([1, 2, 3, 4]).mean()
            # loss_iou = (loss_iou * (1 - iou_mask).reshape(shape=loss_iou.shape)).sum() / ((1 - iou_mask).sum() + 1e-5) * b * 0.05
        ...
        if self.iou_aware_loss is not None:
            ...
            # loss_iou_aware = loss_iou_aware.mean() * b #.sum([1, 2, 3, 4]).mean()
            loss_iou_aware = loss_iou_aware.sum() / num_obj * 0.05 * b  #.sum([1, 2, 3, 4]).mean()
