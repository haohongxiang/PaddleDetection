...
class Block(nn.Module):
    ...
    def forward(self, x, x_rel_pos_bias=None, return_attention=False):
        ...
        # print('x_rel_pos_bias', x_rel_pos_bias, )
...
def prepare_rpe(rel_pos_bias, src_patch_shape, dst_patch_shape):
    # rel_pos_bias = state_dict[key]                    # ((2*14-1)*(2*14-1)+3, 12)     
    src_num_pos, num_attn_heads = rel_pos_bias.size()  # 732
    # dst_num_pos, _ = model.state_dict()[key].size()     # (2*32-1)*(2*32-1)+3 / (2*42-1)*(2*68-1)+3 
    # dst_patch_shape = model.patch_embed.patch_shape     # (42,68)  (H,W)

    # rank, _ = get_dist_info()
    #print("src_patch_shape:", src_patch_shape)
    #print("dst_patch_shape:", dst_patch_shape) 
    dst_num_pos = (dst_patch_shape[0] * 2 - 1) * (dst_patch_shape[1] * 2 - 1
                                                  ) + 3

    if dst_patch_shape[0] != src_patch_shape[0] or dst_patch_shape[
            1] != src_patch_shape[1]:
        ...
        src_size_0, src_size_1 = src_patch_shape[0] * 2 - 1, src_patch_shape[
            1] * 2 - 1
        ...
        rel_pos_bias = rel_pos_bias.reshape(1, src_size_0, src_size_1,
                                            dim).permute(0, 3, 1, 2)
        new_rel_pos_bias = nn.functional.interpolate(
            rel_pos_bias,
            scale_factor=(dst_size_0 / src_size_0, dst_size_1 / dst_size_1),
            mode='bicubic', )
        new_rel_pos_bias = new_rel_pos_bias.permute(0, 2, 3, 1).view(
            1, -1, dim).squeeze(0)
        #print("if new_rel_pos_bias:", new_rel_pos_bias.shape)
    ...
    #print("before new_rel_pos_bias:", new_rel_pos_bias.shape)
    ...
    new_rel_pos_bias = new_rel_pos_bias.permute(
        2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
    #print("new_rel_pos_bias:", new_rel_pos_bias.shape)
...
class VisionTransformerDet(VisionTransformer):
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        tmp = patch_pos_embed.reshape(1, self.patch_embed.num_patches_w,
                                      self.patch_embed.num_patches_h,
                                      dim).permute(0, 3, 1, 2)
        patch_pos_embed = nn.functional.interpolate(
            patch_pos_embed.reshape(1, self.patch_embed.num_patches_w,
                                    self.patch_embed.num_patches_h,
                                    dim).permute(0, 3, 1, 2),
    ...
    def forward(self, x):
        ...
        #print("##############forward pos_emb:", self.pos_embed)
        time_begin = time.time()
        if self.relative_position_bias_table is None:
            x_rpe = None
        else:
            dst_rpe_shape = (Wp, Hp) if H <= W else (Hp, Wp)
            x_rpe = prepare_rpe(self.relative_position_bias_table,
                                self.patch_embed.patch_shape, dst_rpe_shape)
        #print('##3###$###########self.blocks:', len(self.blocks))
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                xp = self.norm(x[:, 1:, :]).permute(0, 2, 1).reshape(B, -1, Hp,
                                                                     Wp)
